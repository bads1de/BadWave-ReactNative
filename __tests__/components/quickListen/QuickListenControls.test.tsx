...
import QuickListenControls from "@/components/quickListen/QuickListenControls";
import Song from "@/types";

describe("QuickListenControls", () => {
  const mockSong: Song = {
    id: "1",
    title: "Test Song",
    author: "Test Artist",
  ...
  describe("ボタンのレンダリング", () => {
    it("いいねボタンを表示すること", () => {
      const { getByTestId } = render(<QuickListenControls song={mockSong} />);
    ...
    it("「フルで聴く」テキストを表示すること", () => {
      const { getByText } = render(<QuickListenControls song={mockSong} />);
  ...
  describe("ボタンのコールバック", () => {
    it("いいねボタン押下時に onLike が呼ばれること", () => {
      ...
      const { getByTestId } = render(
        <QuickListenControls song={mockSong} onLike={onLike} />
      );
    ...
    it("フルで聴くボタン押下時に onPlayFull が呼ばれること", () => {
      ...
      const { getByTestId } = render(
        <QuickListenControls song={mockSong} onPlayFull={onPlayFull} />
      );
