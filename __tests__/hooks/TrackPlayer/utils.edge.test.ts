...
describe('TrackPlayer utils - Edge Cases', () => {
  ...
  beforeEach(() => {
    ...
    jest.spyOn(utils, 'getOfflineStorageService').mockReturnValue(mockOfflineStorageService);
    
    // 各テストで必要なモックを設定
    mockOfflineStorageService.getSongLocalPath.mockImplementation(async (songId) => {
      if (songId === 'song-1' && mockOfflineStorageService.getSongLocalPath.mock.calls.length === 1) {
        return null;
      } else if (songId === 'song-1' && mockOfflineStorageService.getSongLocalPath.mock.calls.length === 2) {
  ...
  describe('convertSongToTrack - Edge Cases', () => {
    ...
    it('should handle errors during local path retrieval', async () => {
      mockOfflineStorageService.getSongLocalPath.mockRejectedValue(new Error('Storage error'));
  ...
  describe('convertToTracks - Edge Cases', () => {
    ...
    it('should handle errors during conversion of individual songs', async () => {
      ...
      jest.spyOn(utils, 'convertSongToTrack')
        .mockImplementationOnce(() => Promise.reject(new Error('Conversion error')))
        .mockImplementationOnce((song) => Promise.resolve({
          id: song.id,
          url: song.song_path,
          title: song.title,
          artist: song.author,
