...
フルスクリーンの動画を背景に、アーティスト情報やアクションボタンを配置する。
...
- **再生動作**: 各動画の再生が終了すると、**次の動画が自動的に再生される**。同じ動画はループせず、リストの最後まで再生されたら停止する。ユーザーが上下に**スワイプ**することでも、前後の動画へ移動できます。
...
- **UIコンポーネントの表示制御**:
  - `Reels`タブで動画が再生される際、**アプリ共通のヘッダーとミニプレイヤーは非表示**にし、没入感を高める。
  - `Reels`再生中は、バックグラウンドの音楽プレイヤー(`TrackPlayer`)を一時停止する。
- **タブ切り替え時の動作**:
  - ユーザーが`Reels`タブから他のタブへ切り替えた場合、**動画の再生を停止し、関連する動画リソースをすべて解放する**。
  - 再び`Reels`タブに戻ってきた際には、リストの先頭から表示を再開する（状態はリセットされる）。
...
- **統合テスト**:
  ...
  - 他のタブに切り替えた際に、動画の再生が停止し、リソースが解放されること。
- **パフォーマンス/E2E テスト**:
