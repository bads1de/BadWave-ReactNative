...
export function useDownloadSong() {
  ...
  return useMutation({
    ...
    onSuccess: (result, song) => {
      if (result.success) {
        // ダウンロード状態のキャッシュを更新
        queryClient.setQueryData([CACHED_QUERIES.downloadStatus, song.id], true);
        // ダウンロード済み曲リストを無効化して再取得を促す
        queryClient.invalidateQueries({ queryKey: [CACHED_QUERIES.downloadedSongs] });
      }
    },
  });
}
...
export function useDeleteDownloadedSong() {
  ...
  return useMutation({
    ...
    onSuccess: (result, songId) => {
      if (result.success) {
        // ダウンロード状態のキャッシュを更新
        queryClient.setQueryData([CACHED_QUERIES.downloadStatus, songId], false);
        // ダウンロード済み曲リストを無効化して再取得を促す
        queryClient.invalidateQueries({ queryKey: [CACHED_QUERIES.downloadedSongs] });
      }
    },
